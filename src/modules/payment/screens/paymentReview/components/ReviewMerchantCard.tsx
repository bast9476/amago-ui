...
export default function ReviewMerchantCard() {
  const selectedMerchant = useAppSelector(selectSelectedMerchant);
  const { handleChangeMerchant } = usePaymentReview();
  const fallbackLogo = merchantLogos.biman;

  const merchantName = selectedMerchant?.name + ' Bangladesh Airlines' || 'Biman Bangladesh Airlines';
  const merchantLogo = selectedMerchant?.logo || fallbackLogo;
  const accountNumber = '01••••••234';
  const accountType = 'Flight';

  return (
    <View className="w-full pb-2.5 mb-4 border-b border-[#f3f4f7]">
      <View className="flex-row items-center gap-3">
        <View className="w-[44px] h-[40px] rounded-[7px] overflow-hidden" style={styles.logoShadow}>
          <Image
            source={merchantLogo}
        ...
        <View className="flex-1">
          <View className="flex-row items-center gap-2 mb-1">
            <Text className="flex-1 text-[15px] font-semibold text-[#242424]" numberOfLines={1}>
              {merchantName}
            </Text>
          ...
          <View className="flex-row items-center gap-2">
            <Text className="text-[15px] text-[#62748e]">{accountNumber}</Text>
            <Text className="text-[15px] text-[#62748e]">{accountType}</Text>
          </View>
          <TouchableOpacity className='mt-2' onPress={handleChangeMerchant}>
            <Text className="text-[14px] font-semibold text-[#00a551]">Change</Text>
          </TouchableOpacity>
        </View>
      </View>
    </View>
